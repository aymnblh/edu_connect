...
async def request_student_link(
    ...
    # Check if already linked without triggering async lazy loading.
    ...
    if linked_res.scalar_one_or_none():
    ...
    stmt = select(VerificationRequest).where(
        ...
        VerificationRequest.school_id == student.school_id,
...
async def link_student_by_qr(
    ...
    if current_user.school_id and pending_link.school_id != current_user.school_id:
    ...
    stmt = select(StudentParent).where(
        StudentParent.school_id == pending_link.school_id,
    ...
    cnt_stmt = select(func.count()).select_from(StudentParent).where(
        StudentParent.school_id == pending_link.school_id,
        StudentParent.student_id == pending_link.student_id,
    )
